...
describe('computeMessageHash', () => {
	it('produces consistent SHA-256 hashes', () => {
		...
		const hash1 = computeMessageHash(msg, 0, '0'.repeat(64))
		const hash2 = computeMessageHash(msg, 0, '0'.repeat(64))
	...
	it('produces different hashes for different messages', () => {
		...
		const hash1 = computeMessageHash({ role: 'user', content: 'hello' }, 0, prev)
		const hash2 = computeMessageHash({ role: 'user', content: 'world' }, 0, prev)
	...
	it('produces different hashes for different indices', () => {
		const msg: Message = { role: 'user', content: 'hello' }
		const prev = '0'.repeat(64)
		const hash1 = computeMessageHash(msg, 0, prev)
		const hash2 = computeMessageHash(msg, 1, prev)

		expect(hash1).not.toBe(hash2)
	})

	it('chains hashes — different previousHash yields different result', () => {
		const msg: Message = { role: 'user', content: 'hello' }
		const hash1 = computeMessageHash(msg, 0, '0'.repeat(64))
		const hash2 = computeMessageHash(msg, 0, 'a'.repeat(64))
...
describe('verifyMessageChain', () => {
	it('verifies empty chain', () => {
		const result = verifyMessageChain([], [])
	...
	it('verifies valid chain', () => {
		...
		const h0 = computeMessageHash(messages[0], 0, zeroHash)
		const h1 = computeMessageHash(messages[1], 1, h0)
		const h2 = computeMessageHash(messages[2], 2, h1)

		const result = verifyMessageChain(messages, [h0, h1, h2])
		expect(result.valid).toBe(true)
	...
	it('detects tampered message', () => {
		...
		const result = verifyMessageChain(messages, [h0, h1])
	...
	it('detects length mismatch', () => {
		...
		const result = verifyMessageChain(messages, [])
		expect(result.valid).toBe(false)
		expect(result.brokenAt).toBe(0)
	})
})
