...
| **Fluent verification** | `agent.withVerifier(v)`, `agent.withRetryPolicy({ maxAttempts, semantic })`, `withVerifiers`, `AgentRetryPolicy` | Chainable, immutable verification on any agent returned by `defineAgent`. Internally loops via `runWithVerification`. Validators operate on `AgentResult`. Zero overhead when no verifier is attached. |
