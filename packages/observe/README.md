...
| **Verifiable Agent Execution** | `createProofRecorder`, `verifyProof`, `compareProofs`, `ProofRecorder`, `ProofSession`, `ExecutionProof`, `ProofEvent`, `VerifyProofResult`, `ReplayDiff`, `ReplayStrategy` | Signed per-run execution proofs with hash-chained events. Build on `@elsium-ai/core` Ed25519 + `WriteOnceStore`. Third parties verify proofs offline with just the public key, and `compareProofs(a, b, { strategy })` diffs two proofs for reproducibility. CLI: `elsium verify` and `elsium replay`. |
| **AI-BOM** | `generateAiBom`, `verifyAiBom`, `diffAiBom`, `passesGate`, `serializeAiBom`, `AiBom`, `AiBomInput`, `AiBomDiff`, `ComponentDrift`, `DriftSeverity`, `VerifyAiBomResult` | Ed25519-signed manifest of what an agent is made of — models, prompts, tools (with sandbox capabilities), MCP servers, eval datasets, policies, thresholds. Deterministic and order-independent; `diffAiBom` ranks composition drift by whether the blast radius grew. Where a proof records one run, a BOM records the composition every run inherits. CLI: `elsium bom verify` and `elsium bom diff`. |
