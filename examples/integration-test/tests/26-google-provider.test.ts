/**
 * Test 26: Google Provider (Real LLM)
 * Verifies: gateway.complete() and stream() with real Google — full LLMResponse shape
 */
import { expect, it } from 'vitest'
import { assertNonEmptyString, createGoogleGateway, describeWithGoogle } from '../lib/helpers'

describeWithGoogle('26 — Google Provider (Real LLM)', () => {
	it('complete() returns a full LLMResponse', async () => {
		const gw = createGoogleGateway()
